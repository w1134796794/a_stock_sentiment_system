...
class MarketMainlineAnalyzer:
    def __init__(self):
        ...
        # 加载行业层级映射 - 使用新的CSV文件
        mapping_csv = Path("data/Industry_Mapping.csv")
        if mapping_csv.exists():
            self.hierarchy_df = pd.read_csv(mapping_csv, encoding='utf-8')
            # 重命名列以兼容原有代码
            self.hierarchy_df.columns = ['L1_Industry', 'L2_Industry', 'L3_Industry']
            print(f"✓ 加载行业映射(CSV): {len(self.hierarchy_df)} 个三级行业")
        else:
            # 回退到旧的Excel文件
            excel_file = self.data_dir / "industry_hierarchy.xlsx"
            if excel_file.exists():
                self.hierarchy_df = pd.read_excel(excel_file)
                print(f"✓ 加载行业映射(Excel): {len(self.hierarchy_df)} 个三级行业")
            else:
                print("✗ 未找到行业映射文件")
                self.hierarchy_df = pd.DataFrame(columns=['L1_Industry', 'L2_Industry', 'L3_Industry'])
        ...
        for _, row in self.hierarchy_df.iterrows():
            l3 = row['L3_Industry']
            self.l3_to_l2_map[l3] = row['L2_Industry']
            self.l3_to_l1_map[l3] = row['L1_Industry']
    ...
    def classify_stock_to_l3(self, industry_name):
        ...
        industry_str = str(industry_name).strip()
        
        # 直接匹配L3
        match = self.hierarchy_df[self.hierarchy_df['L3_Industry'] == industry_str]
        if not match.empty:
            return industry_str
        
        # 尝试匹配L2
        match_l2 = self.hierarchy_df[self.hierarchy_df['L2_Industry'] == industry_str]
        if not match_l2.empty:
            # 返回该L2下的第一个L3
            return match_l2.iloc[0]['L3_Industry']
        
        # 关键词匹配
        for _, row in self.hierarchy_df.iterrows():
            l3 = row['L3_Industry']
            # 完全匹配或包含关系
            if l3 in industry_str or industry_str in l3:
                return l3
            # 匹配L2
            l2 = row['L2_Industry']
            if l2 in industry_str or industry_str in l2:
                return l3
        
        return "其他"
    
    def get_l1_l2_by_l3(self, l3_industry):
        """根据L3获取L1和L2"""
        if l3_industry in self.l3_to_l2_map:
            return self.l3_to_l1_map[l3_industry], self.l3_to_l2_map[l3_industry]
        
        # 从DataFrame查找
        match = self.hierarchy_df[self.hierarchy_df['L3_Industry'] == l3_industry]
        if not match.empty:
            return match.iloc[0]['L1_Industry'], match.iloc[0]['L2_Industry']
