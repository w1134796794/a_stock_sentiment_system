...
# Redis 是生产环境的共享运行态存储。未配置时自动使用进程内存，便于本地单进程开发；
# 服务器多 Worker 部署必须配置 REDIS_URL，避免实时缓存和任务状态各自为政。
REDIS_URL = os.getenv("REDIS_URL", "").strip()
REDIS_KEY_PREFIX = os.getenv("REDIS_KEY_PREFIX", "a_stock").strip() or "a_stock"
