...
class THSSectorTracker:
    ...
    def identify_main_themes(self, trade_date: str,
        ...
        核心思路：
        对所有热点板块（概念+行业）统一用四维评分排名，自然浮出主线，
        不再强行匹配概念-行业对。

        四维评分体系：
        1. 涨停集中度（权重30%）- 板块内涨停家数越多，主线特征越强
        2. 梯队完整性（权重30%）- 首板→二板→三板→高标，梯队越完整越强
        3. 持续性（权重25%）- 连续多日活跃的板块才是真主线
        4. 龙头强度（权重15%）- 龙头连板高度决定板块空间

        Args:
            trade_date: 交易日期（YYYYMMDD）
            hot_concepts_df: 热点概念分析结果
            hot_industries_df: 热点行业分析结果
            concept_persistence_df: 概念持续性分析结果
            industry_persistence_df: 行业持续性分析结果
            top_n: 返回前N条主线

        Returns:
            DataFrame: 主线识别结果，按综合评分降序
        """
        logger.info("=" * 80)
                ...
                for _, row in limit_up_df.iterrows():
                    ...
                    if code in member_codes and height == max_height:
                        if fd_col and pd.notna(row.get(fd_col)):
                            ...
                            if fd_amount > 1e8:  # 封单>1亿
                                fd_bonus = 10
                            elif fd_amount > 5e7:  # 封单>5000万
                                fd_bonus = 5
                        break
