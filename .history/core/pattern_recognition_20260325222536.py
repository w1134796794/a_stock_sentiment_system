...
class PatternRecognition:
            ...
            if hist_60d_dict and code in hist_60d_dict:
                ...
                if hist_data is not None and not hist_data.empty and len(hist_data) >= 20:
                    ...
                    if 'pct_change' in hist_data.columns:
                        ...
                        hist_data['consecutive'] = hist_data['is_limit_up'].rolling(3).sum()
                        max_consecutive = hist_data['consecutive'].max()
                        if max_consecutive < 3:
                            continue
                        
                        # 条件3: 近期缩量（地量）
                        if 'vol' in hist_data.columns:
                            vol_ma20 = hist_data['vol'].tail(20).mean()
                            min_vol_recent = hist_data['vol'].tail(10).min()
                            if min_vol_recent >= vol_ma20 * 0.5:  # 不是地量
                                continue
    ...
    def scan_all_patterns(self, today_date: str, yesterday_date: str) -> Dict[str, List[PatternSignal]]:
        ...
        # 8. 检测龙二波
        results["龙二波"] = self.detect_dragon_second_wave(today_zt)
